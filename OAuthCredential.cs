...
namespace TripIt
{
    ...
    public class OAuthCredential : Credential
    {
        ...
        public String GetSessionParameters(String redirectUrl, String action)
        {
            ...
            {
                list.Add(string.Format("{0}:{1}", Utility.JsonEncode(kvp.Key), Utility.JsonEncode(kvp.Value)));
            }
        ...
        {
            var list = new List<string>();
            var parameters = GenerateOAuthParameters(request.Method, urlBase, args);

            foreach (var kvp in parameters)
            {
                list.Add(string.Format("{0}=\"{1}\"", Utility.UrlEncode(kvp.Key), Utility.UrlEncode(kvp.Value)));
            }
        ...
        private string GenerateSignatureBaseString(string method, string urlBase, Dictionary<string, string> args)
        {
            ...
            string parameters = Utility.UrlEncode(String.Join("&",list));
